...
def get_possible_artists(artist_name):
    ...
    for artist in sorted_items:
        ...
        artist_data.append(
            (artist["name"], artist["id"], image_url, artist["popularity"])
        )
...
def get_most_recent_song(artist_id):
    ...
    releases = sp.artist_albums(
        artist_id, album_type="single,album", country="US", limit=50
    )
    ...
    for album in albums:
        ...
        if tracks["items"]:
            ...
            return latest_track["name"], __format_release_date(
